...
import { Upload, Download, Loader as Loader2, FileText, ChartBar as BarChart3, Database, History, Trash2 } from 'lucide-react';

// Define the type for saved analysis
...
export default function BatchPage() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<BatchResult['predictions'][number] | null>(null);
  const [recentAnalyses, setRecentAnalyses] = useState<SavedAnalysis[]>([]);

  // Load saved analyses from localStorage on mount
  useEffect(() => {
    const saved = localStorage.getItem('batchAnalyses');
    if (saved) {
      try {
  ...
  const saveAnalysis = (analysis: SavedAnalysis) => {
    const updated = [analysis, ...recentAnalyses].slice(0, 20); // Keep last 20
    setRecentAnalyses(updated);
    localStorage.setItem('batchAnalyses', JSON.stringify(updated));
  };

  // Clear all analyses
  const clearHistory = () => {
    setRecentAnalyses([]);
    localStorage.removeItem('batchAnalyses');
  };
  ...
  // Load a saved result
  const loadSavedResult = (savedResult: BatchResult) => {
                  ...
                  <div className="space-y-2 max-h-[300px] overflow-y-auto">
                    {recentAnalyses.map((analysis) => (
                      <div
                        ...
                        onClick={() => loadSavedResult(analysis.result)}
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <p className="font-medium text-sm truncate">{analysis.filename}</p>
                        ...
                        <div className="flex gap-2 mt-2 text-xs">
                          <span className="text-red-600">{analysis.threats_detected} threats</span>
                          <span className="text-muted-foreground">|</span>
                          <span className="text-green-600">{analysis.total_records - analysis.threats_detected} normal</span>
                        </div>
                      </div>
                    ))}
                  </div>
                        ...
                        <Table>
                          ...
                          <TableBody>
                            {result.raw_data.map((row, idx) => (
                              <TableRow key={idx}>
                                {Object.values(row).map((value: unknown, vidx) => (
