...
export default function LivePage() {
  ...
  const fetchPackets = useCallback(async () => {
    try {
      ...
      // Ensure packets have correct status format (Attack/Normal)
      const formattedPackets = newPackets.map((p: any) => ({
        ...p,
        status: p.status === 'attack' ? 'Attack' : 'Normal'
      }));
      
      setPackets(prev => {
        const combined = [...formattedPackets, ...prev].slice(0, 120);
        return combined;
      });

      const now = new Date();
      const timeStr = `${now.getHours()}:${String(now.getMinutes()).padStart(2, '0')}:${String(now.getSeconds()).padStart(2, '0')}`;
      const normalCount = formattedPackets.filter(p => p.status === 'Normal').length;
      const attackCount = formattedPackets.filter(p => p.status === 'Attack').length;
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-600">Real-time traffic monitoring with XGBoost predictions</p>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Model confidence on latest packet</p>
              </CardContent>
            </Card>
          ...
          {trendData.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Traffic Trend (Last 20 samples)</CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    {packets.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={13} className="text-center text-muted-foreground">
                          {isRunning ? 'Waiting for packets...' : 'Click Start to begin monitoring'}
                        </TableCell>
                      </TableRow>
                    ) : (
                      packets.map((packet) => (
                        <TableRow
                          ...
                          className={`${selectedPacket?.id === packet.id ? 'bg-blue-50' : ''} ${
                            packet.status === 'Attack' ? 'bg-red-50 hover:bg-red-100' : ''
                          }`}
                        >
                          <TableCell className="font-mono text-xs">{packet.time.split('T')[1]?.slice(0, 8) || packet.time}</TableCell>
                          ...
                          <TableCell>
                            <Badge variant={packet.status === 'Attack' ? 'destructive' : 'default'}>
                              {packet.status === 'Attack' ? '🔴 ATTACK' : '🟢 NORMAL'}
                            </Badge>
                          </TableCell>
